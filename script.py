...
def shading(symbols):
    ...
    for symbol in symbols:
        ...
        if args[0] == 'light':
            lights[symbol] = args[1]
        if args[0] == 'constants':
            constants[symbol] = args[1]

        if symbol == 'ambient':
            ambient = [int(args[1]), int(args[2]), int(args[3])]

    return {'light': lights, 'constants': constants, 'ambient': ambient}
    ...
    for f in range(num_frames):
        ...
        for command in commands:
            ...
            if c == 'set':
                ...
                draw_polygons(tmp, screen, zb, color, flat)
                tmp = []
            elif c == 'sphere':
                add_sphere(tmp,
